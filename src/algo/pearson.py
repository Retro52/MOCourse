...
def pearson2(_f, _df, _x0: np.ndarray,
             _search_algo: callable = golden_selection.golden_selection,
             _max_iterations: int = 100000,
             _eps: float = 1e-4,
             _sv_q: float = 0.1,
             _gs_e: float = 1e-2,
             _df_h: float = 1e-2,
             _test_option: bool = True,  # true if default, false if it is weird gradient stuff
             _restarts: int = 0):
    _x_p = np.array(_x0)
    _x = _x_p
    ...
    while True:
        ...
        _lam = _search_algo(_f, _inter, _x, _s, _eps=_gs_e)
        ...
        if (np.linalg.norm(_dx) / np.linalg.norm(_x_p)) <= _eps and abs(_f(_x) - _f(_x_p)) <= _eps and _test_option:
            break

        if not _test_option and np.linalg.norm(_dg) < _eps:
            break
        ...
        if np.abs(denominator) == 0 or np.linalg.norm(_s) == 0 or _iter > _max_iterations:
            ...
            log.program_log(log.LogLevel.Warning, f"pearson2: returned because denominator is 0, "
                                                  ...
                                                  f"{np.abs(denominator) == 0:5} | "
                                                  f"{np.linalg.norm(_s) == 0:5} | "
                                                  f"{_iter > _max_iterations:5}")
            break
    ...
    log.program_log(log.LogLevel.Debug, f"pearson2: Search is over:\n"
                                        f"{'Function min point':25} | {'Iterations':10} | "
                                        f"{'Derivative h':12} | {'Search eps':12} | {'Eps':9} | {'Sven q':9} | "
                                        f"{'F':12} \n"
                                        f"{str(_x):25} | {_iter:10} | {_df_h:5.10F} | "
                                        f"{_gs_e:5.10F} | {_eps:5.7F} | {_sv_q:5.7F} | {_f(_x)}\n")
    return _x, _history
